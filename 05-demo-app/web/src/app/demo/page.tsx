...
import { getDemoPhasesSnapshot, getKbSummarySnapshot } from "@/lib/backend";
import { sampleQueries } from "@/lib/demo-data";

export default async function DemoPage() {
  const [kbSummary, phaseSnapshot] = await Promise.all([
  ...
  return (
    <Suspense fallback={<div className="px-6 py-12 text-sm text-slate-200/70">Loading demo workbench...</div>}>
      <DemoClient initialKbSummary={kbSummary} initialPhases={phaseSnapshot.phases} sampleQueries={sampleQueries} />
    </Suspense>
  );
}
